...
class VirtualNode(nn.Module):
    def __init__(self, in_feats, hidden_feats, layers, vn_index, model = "gcn", num=1, edge_index=None, dropout=0.5, residual=False, mlp_share=False):
        ...
        self.mlp_share = mlp_share
        
        if not self.mlp_share:
            self.vn_emb = nn.Embedding(num, in_feats)
        else:
            in_feats = hidden_feats
            self.vn_emb = nn.Embedding(num, hidden_feats)
    ...
    def update_vn_emb(self, layer, x, vx):
        ...
        if not self.mlp_share:
            ...
            vx_temp = self.mlp_vns[0](vx_temp )
...
class VNGNN(torch.nn.Module):
    def __init__(self, 
        ...
        if self.mlp_share:
            if in_channels == hidden_channels:
                self.feat_trans = nn.Identity()
            else:
                self.feat_trans = nn.Linear(in_channels, hidden_channels)
                in_channels = hidden_channels
                
        if self.use_virtual:
            
            self.vn_index, self.cluster_adj = self.build_virtual_edges(sparse_ratio=sparse_ratio)
            self.vns = VirtualNode(in_channels, hidden_channels, num_layers, self.vn_index, model, num_clusters, self.cluster_adj, self.dropout, mlp_share=self.mlp_share)
    ...
    def build_virtual_edges(self, sparse_ratio=1.0):
        vn_index = get_vn_index(
            ...
            1, 
            self.edge_index, 
            save_dir=f"cluster_data/{self.dataset}/{self.dataset}_clutser_{self.clutser_method}_{self.num_clusters}.pt") # (C, N), index[i] specifies which nodes are connected to VN i
    ...
    def build_cluster_edges(self, edge_index, save_dir):
        if os.path.exists(save_dir):
            ...
            node2clutser = vn_indices[:, 1]
            for i in tqdm(range(N)):
                start, end = start_lst[i], end_lst[i]
                c_i, c_j = node2clutser[start], node2clutser[end]
                cluster_adj[c_i][c_j] += 1
            end = time.time()
    ...
    def forward(self, data, batch_train=False):
        ...
        x = self.feat_trans(x) # for mlp share
        if self.use_virtual:
